...
import com.study.rbac.security.model.*;
 ...
 */
public class DataInitTest extends SecurityApplicationTests {
    @Autowired
    ...
    private Role createRole(boolean isAdmin) {
        ...
        role.setCreateTime(DateUtil.current());
        role.setUpdateTime(DateUtil.current());
        roleDao.save(role);
        return role;
    }
    ...
    private User createUser(boolean isAdmin) {
        ...
        user.setUpdateTime(DateUtil.current());
        userDao.save(user);
        return user;
    }
